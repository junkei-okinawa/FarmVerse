...
fn main() -> anyhow::Result<()> {
    ...
    // settings: min=3300, max=4200 の場合、3500mV は約 22% に相当する ( (3500-3300)/(4200-3300) )。
    // ただし、実際の運用では以下の要因で電圧がさらに低下する／誤差が生じる:
    //   - WiFi送信やCPU負荷時の瞬間的な電圧降下
    //   - ADC測定誤差・キャリブレーション誤差
    //   - バッテリーの経年劣化や内部抵抗の増加
    //   - 温度変化や配線ロスなどのばらつき
    // このため、「3.5V付近で不安定になり始める」という約22%の理論値よりも
    // 余裕を持たせたしきい値として 30% を採用し、「30% 以下 (<= 30)」を
    // 低電圧によるスキップ対象とすることで、起動失敗や誤動作をより安全側に避ける。
