...
/// メインデータ処理ループ
///
/// データキューからデータを取り出してUSB経由で送信し、
/// 同時にUSBからのコマンドを監視してESP-NOW送信を実行します。
fn process_data_loop(usb_cdc: &mut UsbCdc, esp_now_sender: &mut EspNowSender) -> Result<()> {
    info!("Entering main processing loop...");

    loop {
        // 1. キューからデータを取得してUSB送信
...
fn main() -> Result<()> {
    ...
    // カメラはすでに register_esp_now_peers() で登録済みなので、
    // EspNowSender では重複登録を避ける
    info!("ESP-NOW sender initialized. Peers already registered by register_esp_now_peers().");
