...
extern "C" fn esp_now_recv_cb(
    ...
    let mut callback = |received_data| {
        ...
        match queue::data_queue::dequeue() {
            Ok(received_data) => {
                ...
                match result {
                    ...
                    Err(e) => {
                        warn!("Streaming Loop: Failed to process data via Streaming Controller for {}: {}",
                              mac_str, e);
                        
                        // フォールバック: 従来のUSB送信方式
                        match usb_cdc.send_frame(&received_data.data, &mac_str) {
                            Ok(bytes_sent) => {
                                debug!("Streaming Loop: Fallback USB transfer successful: {} bytes", bytes_sent);
                            }
                            Err(usb_err) => {
                                error!("Streaming Loop: Both streaming and fallback failed for {}: {}", mac_str, usb_err);
                            }
                        }
                    }
                }", mac_address, e);
...
fn main() -> Result<()> {
    ...
    info!("Streaming Controller: {} devices registered", streaming_controller.list_devices().len());
