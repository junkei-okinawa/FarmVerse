...
from processors.sleep_controller import (
...
class StreamingSerialProtocol(asyncio.Protocol):
    ...
    def data_received(self, data):
        ...
        if config.DEBUG_FRAME_PARSING:
            logger.debug(
                f"Received {len(data)} bytes: {data.hex() if len(data) < 100 else data[:50].hex() + '...'}"
            )
            logger.debug(
                f"Buffer size before: {len(self.buffer)}, after: {len(self.buffer) + len(data)}"
            )
        ...
        while True:
            ...
            if start_index == -1:
                if config.DEBUG_FRAME_PARSING and len(self.buffer) > 0:
                    logger.debug(
                        f"No START_MARKER found in buffer of {len(self.buffer)} bytes"
                    )
            ...
            if start_index > 0:
                ...
                if config.SUPPRESS_DISCARD_LOGS:
                    logger.debug(
                        f"Discarding {start_index} bytes: {discarded_data.hex()}"
                    )
                else:
                    logger.warning(
                        f"Discarding {start_index} bytes: {discarded_data.hex()}"
            ...
            if config.DEBUG_FRAME_PARSING:
                logger.debug(
                    f"Found START_MARKER at index 0, buffer size: {len(self.buffer)}"
                )
            ...
            if len(self.buffer) < header_length:
                if config.DEBUG_FRAME_PARSING:
                    logger.debug(
                        f"Insufficient data for header: {len(self.buffer)} < {header_length}"
                    )
                break

            try:
                # ヘッダーを解析
                sender_mac, frame_type, seq_num, data_len = FrameParser.parse_header(
                    self.buffer, 0
                )
                ...
                if config.DEBUG_FRAME_PARSING:
                    ...
                    logger.debug(
                        f"Parsed frame header: {frame_type_name} from {sender_mac}, seq: {seq_num}, data_len: {data_len}"
                    )

                # フレーム検証
                header_start = len(START_MARKER)
                mac_bytes = self.buffer[
                    header_start : header_start + MAC_ADDRESS_LENGTH
                ]
                FrameParser.validate_frame_data(
                    data_len, mac_bytes, config.MAX_DATA_LEN
                )
                ...
                if config.DEBUG_FRAME_PARSING:
                    logger.debug(
                        f"Buffer content around error: {self.buffer[:50].hex()}"
                    )
            ...
            # フレーム全体の長さを計算
            frame_end_index = (
                header_length + data_len + CHECKSUM_LENGTH + len(END_MARKER)
            )

            if config.DEBUG_FRAME_PARSING:
                logger.debug(
                    f"Frame calculation: header_len={header_length}, data_len={data_len}, "
            ...
            if len(self.buffer) < frame_end_index:
                if config.DEBUG_FRAME_PARSING:
                    logger.debug(
                        f"Waiting for complete frame: {len(self.buffer)} < {frame_end_index}"
                    )
                break  # 完全なフレームを待つ
            ...
            if footer != END_MARKER:
                logger.warning(
                    f"Invalid end marker for {sender_mac}, expected: {END_MARKER.hex()}, got: {footer.hex()}"
                )
                if config.DEBUG_FRAME_PARSING:
                    logger.debug(
                        f"Frame details - sender_mac: {sender_mac}, frame_type: {frame_type}, seq_num: {seq_num}, data_len: {data_len}"
                    )
                    logger.debug(
                        f"Buffer dump around end marker (±20 bytes): {self.buffer[max(0, end_marker_start - 20) : end_marker_start + 20].hex()}"
                    )
                await self._handle_frame_error()
                continue
            ...
            if config.DEBUG_FRAME_PARSING:
                frame_type_name = self._get_frame_type_name(frame_type)
                logger.debug(
                    f"Processed {frame_type_name} frame from {sender_mac} (seq: {seq_num}, data_len: {data_len})"
                )
                if frame_type == FRAME_TYPE_EOF:
                    logger.info(f"✓ EOF frame successfully processed for {sender_mac}")

    async def _process_frame_by_type(
        self, sender_mac: str, frame_type: int, seq_num: int, chunk_data: bytes
    ):
        """フレームタイプ別処理"""
        if config.DEBUG_FRAME_PARSING:
            frame_type_name = self._get_frame_type_name(frame_type)
            logger.debug(
                f"Processing {frame_type_name} frame from {sender_mac} (seq: {seq_num}, data_len: {len(chunk_data)})"
            )
    ...
    async def _process_streaming_hash_frame(self, sender_mac: str, chunk_data: bytes):
        ...
        temperature = DataParser.extract_temperature_with_validation(
            temp_log_entry, sender_mac
        )
        ...
        if has_image_data:
            logger.info(
                f"Image data expected for {sender_mac} (hash: {hash_value[:16]}...)"
            )
        else:
            logger.info(
        ...
        if has_image_data:
            ...
            logger.info(
                f"Will send sleep command after EOF for {sender_mac} (image data expected)"
            )
        else:
            logger.info(
                f"Will send sleep command after EOF for {sender_mac} (no image data - dummy hash)"
            )

    async def _process_streaming_data_frame(
        ...
        if len(chunk_data) > HEADER_LENGTH and chunk_data.startswith(START_MARKER):
            ...
            if config.DEBUG_FRAME_PARSING:
                logger.debug(
                    f"Possible nested frame detected in DATA payload from {sender_mac}"
                )

            try:
                # 内部フレームのヘッダー解析
                inner_mac, inner_type, inner_seq, inner_len = FrameParser.parse_header(
                    chunk_data, 0
                )
                ...
                if len(chunk_data) >= required_len:
                    ...
                    potential_end_marker = chunk_data[
                        end_marker_pos : end_marker_pos + len(END_MARKER)
                    ]
                    ...
                    if potential_end_marker == END_MARKER:
                        ...
                        if config.DEBUG_FRAME_PARSING:
                            logger.info(
                                f"Successfully unpacked nested frame: type={inner_type}, len={inner_len} from {sender_mac}"
                            )
                        ...
                        if inner_mac == sender_mac:
                            ...
                            await self._process_frame_by_type(
                                sender_mac, inner_type, inner_seq, inner_payload
                            )
                            return
                        ...
                        logger.debug(
                            f"Nested frame candidates failed footer check: expected {END_MARKER.hex()}, got {potential_end_marker.hex()}"
                        )
            except ValueError as e:
                # パース失敗時は通常のデータとして扱う（偶然START_MARKERと一致した場合など）
                if config.DEBUG_FRAME_PARSING:
                    logger.debug(
                        f"Failed to unpack nested frame, treating as raw data: {e}"
                    )

        # ストリーミングプロセッサーでチャンク処理
        success = await self.streaming_processor.process_chunk(
            sender_mac, chunk_data, seq_num, callback=self._chunk_processed_callback
        )
        ...
        if success:
            ...
            # 統計更新（下位互換性）
            self.stats["total_bytes"] = self.stats.get("total_bytes", 0) + len(
                chunk_data
            )
        else:
    ...
    async def _process_streaming_eof_frame(self, sender_mac: str):
        ...
        if sender_mac in self.eof_processed:
            ...
            if time_diff < 5.0:
                logger.info(
                    f"EOF already processed for {sender_mac} {time_diff:.1f}s ago, skipping duplicate"
                )
                return
    ...
    async def _send_sleep_command(self, sender_mac: str, voltage: float):
        ...
        self.sleep_command_sent = {
            mac: timestamp
            for mac, timestamp in self.sleep_command_sent.items()
        ...
        self.eof_processed = {
            mac: timestamp
            for mac, timestamp in self.eof_processed.items()
            if timestamp >= cleanup_threshold
        }
        ...
        if sender_mac in self.sleep_command_sent:
            ...
            if time_diff < 10.0:
                logger.info(
                    f"Sleep command already sent to {sender_mac} {time_diff:.1f}s ago, skipping duplicate"
                )
                return
        ...
        if self.transport:
            try:
                ...
                logger.info(
                    f"Sent sleep command for {sender_mac}: {command_to_gateway.strip()}"
                )
                # 送信履歴を記録
    ...
    async def _send_sleep_command_after_eof(self, sender_mac: str):
        ...
        if sender_mac not in self.voltage_cache:
            logger.warning(
                f"No voltage cache for {sender_mac}, cannot send sleep command"
            )
            return
        ...
        logger.info(
            f"Retrieved voltage from cache for {sender_mac}: {voltage}% (type: {type(voltage)})"
        )
        ...
        if has_image_data:
            logger.info(
                f"Will send sleep command after EOF for {sender_mac} (with image data, voltage: {voltage}%)"
            )
        else:
            logger.info(
                f"Will send sleep command after EOF for {sender_mac} (no image data - dummy hash, voltage: {voltage}%)"
            )

        # xiaがスリープコマンド待機状態に入るまで待機
        delay_seconds = 2
        logger.info(
            f"Waiting {delay_seconds} seconds for {sender_mac} to enter sleep command reception mode..."
        )
